...
local function UpdateCamera()
	...
	if not IsFreecamFrozen() then
		...
		local lookX
		local lookY
		-- Always allow camera controls when freecam is active
		-- Check for RMB when NUI is focused, otherwise free look
		if IsNuiFocused() then
			if IsDisabledControlPressed(0, 25) or IsControlPressed(0, 25) then -- Try both control methods for RMB
				lookX = GetDisabledControlNormal(0, CONTROLS.LOOK_X)
				lookY = GetDisabledControlNormal(0, CONTROLS.LOOK_Y)
				-- Also try regular controls if disabled doesn't work
				if lookX == 0 and lookY == 0 then
					lookX = GetControlNormal(0, CONTROLS.LOOK_X)
					lookY = GetControlNormal(0, CONTROLS.LOOK_Y)
				end
		          else
		              lookX = 0
		              lookY = 0
...
function StartFreecamThread()
	...
	CreateThread(function()
		...
		if IsNuiFocused() or placing or editingObjectData then
			BeginScaleformMovieMethod(scaleform, "SET_DATA_SLOT")
